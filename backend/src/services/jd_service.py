...
async def tailor_jd(
    ...
    # 3. Load career profile and export as Markdown for LLM consumption
    ...
    career_markdown = ""
    if career_profile:
        career_markdown = await export_markdown(session, career_profile.id)

    # Build legacy career_assets dict (for backward compat with nodes that still use it)
    ...
    try:
        ...
        pipeline_result = {
            ...
            "keyword_coverage": {"coverage_score": 0.0, "uncovered_keywords": []},
            "resume_patches": [],
            "agent_logs": [{"node": "pipeline", "error": str(e)}],
        }

    # 5. Derive match_scores from keyword coverage (Phase 3)
